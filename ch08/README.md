...
工程直觉先给你（不需要任何语言背景）：**封装和抽象天天用，继承省着用**。

### 8.0.1 概念词典（先认识名词，后文反复出现）

| 名词 | 含义 | 记忆钩子 |
|------|------|------|
| **类** class | 造对象的图纸：声明这类东西有哪些数据和哪些行为 | 模具 |
| **对象 / 实例** object / instance | 按图纸造出来的具体东西，互不干扰 | 倒出来的产品 |
| **实例化** instantiate | 类名加括号造对象：`Dog("旺财", 3)` | 开模 |
| **属性** attribute | 挂在对象身上的数据：`self.name`、`d.age` | 产品身上的标签 |
| **方法** method | 类里定义的函数，能操作自身数据：`d.bark()` | 产品自带的功能 |
| **`self`** | 方法里的第一个参数 = "被点出来的那个对象" | "我" |
| **`__init__`** | 初始化方法：造对象时自动调用，负责装属性 | 出厂设置 |
| **实例属性** | 每个对象一份，互不干扰 | 各人的名牌 |
| **类属性** | 全体实例共享一份，适合常量 | 全家共用的大门钥匙 |
| **接口** interface | 类对外承诺的"方法名 + 参数 + 行为" | 柜台窗口 |
| **不变量** invariant | 必须永远为真的规则（如"余额 ≥ 0"），靠方法守门 | 财务铁律 |
...
| 角色 | 是什么 |
|------|------|
| `class Dog:` | 图纸:声明"狗都有名字、年龄,会叫" |
| `self.name = name` | 属性:造对象时装到它身上的数据 |
| `def bark(self)` | 方法:会操作自身数据的函数 |
...
class Dog:
    legs = 4                        # 类属性(全体共享)
    def __init__(self, name):
        self.name = name            # 实例属性(每对象一份)
    def rename(self, new):
        old = self.name             # 局部变量(只在本次方法调用里活)
        self.name = new
```

| | 类属性 | 实例属性 | 局部变量 |
|------|------|------|------|
| 谁有 | 类本身,全体共享 | 每个对象一份 | 只在函数调用期间 |
| 写法 | 类体里、方法外 | `self.xxx = ...` | 普通赋值 |
| 生命周期 | 程序运行全程 | 跟着对象 | 函数返回就没了 |
| ⚠️ 坑 | 实例赋值只是"盖住" | 忘了 self. 就是局部变量 | 想改实例属性必须写 self. |

## 8.3 封装(一):打包——数据和规则住在一起

封装有**两层含义**。第一层:**打包**。

先认识一个词:**不变量(invariant)= 这类对象必须永远满足的规则**。
银行账户的不变量:"余额永远 ≥ 0、存款必须为正"。
封装的核心任务就是守护不变量。
...
def deposit(balance, amount):
    if amount <= 0:
        raise ValueError(...)       # 不变量在这里守一遍
    return balance + amount

def withdraw(balance, amount):
    if amount <= 0:                 # 同一条不变量再守一遍
        raise ValueError(...)
    if amount > balance:            # 每个碰钱的函数都要记得守
        raise ValueError(...)
    return balance - amount

# 调用方直接拿 balance 变量乱改:balance = -99999,不变量碎了,没人拦得住
```

**数据和规则分离,每个函数各自校验,漏一处就是洞;调用方还绕过所有函数直接改数据。**

类的答案:把数据和行为装进同一个盒子,**不变量只写一遍**:
...
acc.balance = -99999        # ❌ 语法完全合法!绕过所有规则,不变量碎了
```

Python **默认不设防**——所有属性公开。Python 的应对是一个约定,两档写法:
...
```
┌─────────── BankAccount 公司 ───────────┐
│   员工:deposit() withdraw() balance()  │  ← 类自己的方法 = 内部
└────────────────────────────────────────┘
        ↓
   顾客:类外面调用它的代码 = 外部
```

- **单下划线 `_balance`**:一块"员工专用"的牌子。语法上照样
  `acc._balance = -1`——门没锁,但全社区看到下划线都懂:"这是内部实现,
  碰了出事自己负责"。
- **双下划线 `__balance`**:Python 悄悄改名为 `_BankAccount__balance`
  (name mangling),外部误碰变难——但主要目的是避免子类意外覆盖属性名,
  **不是真私有**。入门统一用单下划线。

**为什么牌子不是锁也合理?** 想清楚 `_` 到底在保护什么——

> **`_` 防的不是黑客,防的是"无心的错误"和"三个月后的自己"。**
>
> - 同事想改余额:看到 `_` 就知道"哦,内部货",去找公开方法 → 不变量守住 ✅
>   (没有 `_`,他顺手 `acc.balance += 1`,根本不知道有校验,无心绕过 ❌)
> - 排查 bug:修改入口只有 deposit/withdraw 两个 → 两分钟定位
>   (公开属性:每个直接赋值点都是嫌疑人)
>
> **保护的具体形态:合法修改入口从 N 条收敛到 1 条,漏洞面随之缩小。**
> 制度的力量不在门锁,在于"修改必须走唯一的有规则把守的通道"。

**为什么 Python 不上锁?** 哲学:"We're all consenting adults"
(大家都是成年人)——约定 + code review 替代强制拦截。代价是约束力弱,
好处是轻便(写测试碰内部状态不用先写一堆访问器)。
**用下划线表达意图,review 时它就是红线。**

**封装完整定义(两层合起来):打包 = 数据和行为住在一起、不变量只写一遍;
隐藏 = 数据标 `_`、修改入口收敛成方法。**
...
- **父类 / 基类**(parent / base class):被继承的那个(Animal)
- **子类**(child / subclass):继承别人的那个(Dog)——"Dog 是一种 Animal"(is-a)
...
Python 是动态语言:**没有"必须声明实现了什么"的步骤**,方法在,就能用。

**代价**(03 示例演示过):对象没有那个方法,Python 不拦,炸在运行时
(AttributeError)——炸点后移,靠测试兜底。**这就是为什么本章测试更重要**:
语法不帮你查的,测试来查。

## 8.9 方法补充:classmethod 认个脸

本章的主角全是**实例方法**(第一个参数 self,绑在对象上)。
还有一种**类方法**(classmethod):第一个参数是类本身(习惯命名 cls),绑在类上:
...
class BankAccount:
    ...
    @classmethod
    def from_json(cls, text):           # cls 就是 BankAccount 类自己
        import json
        data = json.loads(text)
        return cls(data["owner"], data["balance"])   # cls(...) = BankAccount(...)

acc = BankAccount.from_json('{"owner": "小明", "balance": 100}')
```

最常见用途:**替代构造器**——"从另一种原料造对象"的第二个入口
(这里是从 json 文本造)。注意它和隐藏属性没有关系:`_balance` 的修改权限
看的是"代码在不在类的方法里",不是看方法是不是 classmethod。
本章练习全部用实例方法就能完成,classmethod 见过、认识即可。

## 8.10 dataclass:纯数据类的语法糖(顺手认识)
...
类型注解 `x: float` 主要给编辑器/basedpyright 看(运行时不强制)。
练习先手写 `__init__` 打底,dataclass 当彩蛋认识。

## 8.11 本章小结(对着 8.0 的地图验收)

- **类 = 数据(实例属性)+ 行为(方法)的打包;`__init__` 初始化,self = 被点名的对象**
- `d.bark()` ≡ `Dog.bark(d)`——self 不用传,但必须写
- 实例属性每对象一份(跑一行装一个,写了才有);类属性全体共享(放常量);
  `d1.legs = 3` 只是盖住,不改类;忘写 `self.` 就是局部变量
- **封装**:打包(不变量只写一遍)+ 隐藏(`_` 约定 = "员工专用"的牌子 + 方法守门;
  防无心之失,把修改入口收敛成唯一通道,不防黑客)
- **抽象**:对外只承诺"能做什么";检验标准 = 换实现不改调用方
- `__repr__` 给开发者(优先写它),`__str__` 给用户(友好版)
- **继承**:is-a 才继承,通常先想组合;重写就近覆盖;`super().__init__()` 借父类工序
- **多态**:调用方依赖能力不依赖血统;长 isinstance 链 = 该上多态的信号
- **鸭子类型**:多态的极致,连继承都不用;代价是炸点移到运行时,用测试补位
- classmethod:绑在类上的方法,常见用途是替代构造器(认识即可)
- dataclass:纯数据类的语法糖
